...
export class Compiler {
        ...
        if (stat.init) {
          const init: Statement = /Expression$/.test(stat.init.type)
            ? {
                ...
                expression: stat.init as Expression,
              }
  ...
  private compileExpression(expr: Expression) {
    switch (expr.type) {
      ...
      case "BinaryExpression": {
        ...
        switch (expr.operator) {
          case "==":
            this.writeOp(OpCode.EQ);
            break;
          case "!=":
            this.writeOp(OpCode.NEQ);
            break;
          case "===":
            this.writeOp(OpCode.SEQ);
            break;
          case "!==":
            this.writeOp(OpCode.SNEQ);
            break;
          case "<":
            this.writeOp(OpCode.LT);
            break;
          case "<=":
            this.writeOp(OpCode.LTE);
            break;
          case ">":
            this.writeOp(OpCode.GT);
            break;
          case ">=":
            this.writeOp(OpCode.GTE);
            break;
          case "<<":
            this.writeOp(OpCode.LSHIFT);
            break;
          case ">>":
            this.writeOp(OpCode.RSHIFT);
            break;
          case ">>>":
            this.writeOp(OpCode.URSHIFT);
            break;
          case "+":
            this.writeOp(OpCode.ADD);
            break;
          case "-":
            ...
            break;
          case "*":
            this.writeOp(OpCode.MUL);
            break;
          case "**":
            this.writeOp(OpCode.EXP);
            break;
          case "/":
            this.writeOp(OpCode.DIV);
            break;
          case "%":
            this.writeOp(OpCode.MOD);
            break;
          case "|":
            this.writeOp(OpCode.BOR);
            break;
          case "^":
            this.writeOp(OpCode.BXOR);
            break;
          case "&":
            this.writeOp(OpCode.BAND);
            break;
          case "in":
            this.writeOp(OpCode.IN);
            break;
          case "instanceof":
            this.writeOp(OpCode.INSOF);
            break;
        }
        break;
      }
  ...
  show() {
    ...
    for (const inst of this.instructions) {
      switch (inst.type) {
        ...
        case "opcode": {
          lines.push(
            `\t${OpCode[inst.opcode]}(${inst.opcode
              .toString(16)
              .padStart(2, "0")})${inst.comment ? ` # ${inst.comment}` : ""}`
          );
          break;
        }
        case "data": {
          lines.push(
            `\t${JSON.stringify(inst.rawData)} (${inst.data
  ...
  toNumberArray(): number[] {
    ...
    for (const inst of this.instructions) {
      switch (inst.type) {
        case "label": {
          const label = labelMap.get(inst.name) || {
            address: 0,
            references: [],
          };
